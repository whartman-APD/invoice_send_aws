...
| Sync processes | `--sync-processes` | Upserts Robocorp processes/assistants into Azure SQL (`dbo.dim_processes`). | **AWS** ECS Fargate — daily 4:00 AM Pacific, in the n8n VPC's private subnets so it reaches Azure SQL through the allowed NAT IP (44.253.27.41). *Until cutover: local Docker + Windows Task Scheduler.* |
...
docker-compose run --rm invoice-processor --sync-processes   # only until the AWS cutover
